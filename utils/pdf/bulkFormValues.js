...
function buildMedicalExamReportValues(actualForm, patient) {
  ...
  const values = {
    ...
    companyName: src.companyName || patient?.company || "",
    age: String(src.age || patient?.age || ""),
    department: src.department || patient?.department || "",
    identificationMark: src.identificationMark || patient?.identificationMark || preMed.identificationMark || "",
    bloodGroup: src.bloodGroup || patient?.bloodGroup || preMed.bloodGroup || "",
    rhType: src.rhType || "Ve",
    height: src.height || preMed.height || "",
    weight: src.weight || preMed.weight || "",
    temperature: src.temperature || preMed.temperature || "",
    pulse: src.pulse || preMed.pulse || "",
    bpSystolic: src.bpSystolic || preMed.bpSystolic || "",
    bpDiastolic: src.bpDiastolic || preMed.bpDiastolic || "",
    vaccinated: src.vaccinated || "Yes",
    rs: src.rs || "",
    cvs: src.cvs || "",
    cns: src.cns || "",
    pa: src.pa || "",
    vision: src.vision || preMed.distanceRightVision || "",
    skin: src.skin || "",
    ent: src.ent || "",
    nails: src.nails || "",
